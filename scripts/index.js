document.addEventListener("DOMContentLoaded", async () => {
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    // add classNames specified in data.js object to door element
    let ClassNames = data[i].classNames.trim().split(" ");
    ClassNames[0] != "" && door.classList.add(...ClassNames);
...
const handleClick = (event) => {
  ...
  if (true) {
    // date check etc.
    window.location.href = "./door.html?d=" + event.target.dataset.num;
  }
};
